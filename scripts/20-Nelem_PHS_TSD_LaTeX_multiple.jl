...
d = 0.006
# J = 0.17
J_range = [0.0, 0.05, 0.1, 0.15, 0.16, 0.165, 0.17, 0.175, 0.19]
...
t_start_PHS, t_end_PHS, t_N_PHS = t_end_preintegrate, t_end_preintegrate+10000, 10000
...
sol_J = article1.moded_integrate_multiple_elements(U₀, t_span_preintegrate, d, N_elements, 0.0)
new_U₀ = sol_J[:,end]

for (ii, J) in enumerate(J_range)
    global new_U₀, sol_J
    
    sol_J = article1.moded_integrate_multiple_elements(new_U₀, t_span_preintegrate, d, N_elements, J)
    new_U₀ = sol_J[:,end]
    ...
    limits!(ax_12, t_span_PHS[2]-10000, t_span_PHS[2], nothing, nothing)

    # axislegend(ax, position=:rb) # (l, r, c), (b, t, c)

    savepath = plotsdir("tmp", "20-Nelem_PHS_TSD_LaTeX_multiple_$(lpad(ii,3,"0")).png")
