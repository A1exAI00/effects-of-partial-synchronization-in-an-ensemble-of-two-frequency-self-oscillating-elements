#=
Цель программы: 
Построить пространственно-временную диаграмму для кольца из N_elements=7 элементов.
Задаётся значение параметра d, параметр J.
Постедовательно frames_N раз интегрируется кольцо элементов с сохранением состояния, для 
каждого раза сохраняется отдельный график.
=#
