...
savedir = joinpath("tmp", "26-2elem_slowsys_integrate")
...
J = 0.0
d_start, d_end, d_N = 0.0, 0.03, 101

t_start, t_end = 0.0, 50000.0
...
function is_in_slow_movement(x, y)
    if (article1.A₃[1] < x < article1.B₃[1]) && (article1.B₃[2] < y < article1.A₃[2])
        return true
    end
    if (article1.D₃[1] < x < article1.C₃[1]) && (article1.C₃[2] < y < article1.D₃[2])
        return true
    end
    if (article1.A₁[1] < x < article1.B₁[1]) && (article1.B₁[2] < y < article1.A₁[2])
        return true
...
function filter_slow_movement(sol_x1, sol_x2, sol_y1, sol_y2)
    ...
    for i in eachindex(sol_x1)
        x1, y1 = sol_x1[i], sol_y1[i]
        x2, y2 = sol_x2[i], sol_y2[i]
        if is_in_slow_movement(x1, y1) && is_in_slow_movement(x2, y2)
            push!(sol_x1_slow, x1)
            push!(sol_y1_slow, y1)
            push!(sol_x2_slow, x2)
            push!(sol_y2_slow, y2)
...
sol_preintegrate = article1.moded_integrate_two_elements(U₀, t_span, 0.0, J)
...
for (i,d) in enumerate(d_range)
    ...
    sol_u₁_slow, sol_u₂_slow, sol_v₁_slow, sol_v₂_slow = sol_u₁, sol_u₂, sol_v₁, sol_v₂
    ...
    ax = beautiful_Axis(fig[1, 1], 
        title="Проекция ФП подсистемы медленных движений на (u₁,u₂) \nd=$d, J=$J", 
        xlabel="u₁", ylabel="u₂"
    )
    ...
    scatter!(ax, sol_u₁_slow, sol_u₂_slow, alpha=0.1, markersize=2)

    savepath = plotsdir(savedir, "26-2elem_slowsys_integrate_$(lpad(i,3,"0")).png")
