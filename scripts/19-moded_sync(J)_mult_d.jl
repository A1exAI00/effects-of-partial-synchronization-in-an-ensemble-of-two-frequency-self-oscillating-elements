...
for (j,d) in enumerate(d_range)
    ...
    for (i,J) in enumerate(J_range)
        ...
        t_timer = time()
        eta_curr = (J_N*d_N - (j*J_N+i))*mean(Δt_to_avg)
        println("$i/$J_N, $j/$d_N: Δt=$(round(Δt_timer_curr, digits=5)), eta=$(round(eta_curr, digits=5))")
    ...
    push!(n_sync_total, n_sync)
        
end

fig = Figure(size=(1000, 700))
ax = beautiful_Axis(fig[1, 1], 
    title="Зависимость количества элементов в синх. части от параметра J; φ-$φ_mode, t_int=$t_end, ω_threshold=$ω_threshold", 
    xlabel="J", ylabel="n"
)
...
for i in eachindex(d_range)
    lines!(ax, J_range, n_sync_total[i], label="d=$(d_range[i])")
end

axislegend(ax, position=:rb) # (l, r, c), (b, t, c)
savepath = plotsdir("19-moded_sync(J)_mult_d_$(time()).png")
